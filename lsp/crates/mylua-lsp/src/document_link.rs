...
//!   string content itself, and the target URI is whatever
//!   `resolve_module_to_uri` produces (which already handles
//!   aliases).
...
fn collect_recursive(
    ...
    if node.kind() == "function_call" {
        if let Some((string_node, module_path)) = extract_require_argument(node, source) {
            if let Some(target) = index.resolve_module_to_uri(&module_path) {
