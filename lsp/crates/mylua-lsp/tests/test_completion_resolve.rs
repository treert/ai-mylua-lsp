...
fn completion_resolve_enriches_global_with_detail() {
    ...
    let resolved = completion::resolve_completion(bar, &agg);
    assert!(
...
fn completion_resolve_enriches_local_with_type() {
    ...
    let resolved = completion::resolve_completion(foo, &agg);
    let detail = resolved
        .detail
        .as_deref()
...
fn completion_resolve_preserves_items_without_data() {
    ...
    let resolved = completion::resolve_completion(keyword_item.clone(), &agg);
...
fn completion_resolve_function_adds_markdown_signature() {
    ...
    let resolved = completion::resolve_completion(item, &agg);
