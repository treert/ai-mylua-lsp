...
print(myVar)"#;
    ...
    let result = goto::goto_definition(&doc, &uri, pos(1, 6), &mut agg);
...
local x = foo()"#;
    ...
    let result = goto::goto_definition(&doc, &uri, pos(3, 10), &mut agg);
...
fn goto_parameter() {
    ...
    let result = goto::goto_definition(&doc, &uri, pos(1, 10), &mut agg);
...
fn goto_no_result_for_undefined() {
    ...
    let result = goto::goto_definition(&doc, &uri, pos(0, 10), &mut agg);
...
do
    ...
    let result2 = goto::goto_definition(&doc, &uri, pos(4, 10), &mut agg);
