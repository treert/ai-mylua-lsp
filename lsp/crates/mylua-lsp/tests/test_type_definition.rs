...
"#;
    ...
    let r = goto::goto_type_definition(&doc, &uri, pos(6, 6), &mut agg, &GotoStrategy::Auto)
...
"#;
    ...
    let r = goto::goto_type_definition(&doc, &uri, pos(3, 11), &mut agg, &GotoStrategy::Auto)
        .expect("type definition should resolve");
    let loc = single_loc(&r);
...
fn type_definition_falls_back_to_definition_for_primitive() {
    ...
    let r = goto::goto_type_definition(&doc, &uri, pos(1, 6), &mut agg, &GotoStrategy::Auto)
...
"#;
    ...
    let r = goto::goto_type_definition(&doc, &uri, pos(7, 6), &mut agg, &GotoStrategy::Auto)
