...
"#;
    ...
    let r = goto::goto_type_definition(&doc, intern(uri.clone()), pos(6, 6), &mut agg, &GotoStrategy::Auto)
...
"#;
    ...
    let r = goto::goto_type_definition(&doc, intern(uri.clone()), pos(3, 11), &mut agg, &GotoStrategy::Auto)
        .expect("type definition should resolve");
    let loc = single_loc(&r);
...
fn type_definition_falls_back_to_definition_for_primitive() {
    ...
    let r = goto::goto_type_definition(&doc, intern(uri.clone()), pos(1, 6), &mut agg, &GotoStrategy::Auto)
...
"#;
    ...
    let r = goto::goto_type_definition(&doc, intern(uri.clone()), pos(7, 6), &mut agg, &GotoStrategy::Auto)
...
fn type_definition_unknown_returns_none_after_fallback_also_fails() {
    ...
    let r = goto::goto_type_definition(&doc, intern(uri.clone()), pos(0, 0), &mut agg, &GotoStrategy::Auto);
    assert!(r.is_none());
}
