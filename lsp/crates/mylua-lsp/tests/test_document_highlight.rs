...
fn document_highlight_local_read_and_write() {
    ...
    let src = "local x = 1\nx = 2\nprint(x)\n";
    let (doc, _uri, _agg) = setup_single_file(src, "rw.lua");

    // Click on the first `x`
    let h = document_highlight(&doc, pos(0, 6)).expect("Some");
    let t = triples(&h);
    assert_eq!(
        t,
        vec![
...
fn document_highlight_parameter_is_write() {
    ...
    let (doc, _uri, _agg) = setup_single_file(src, "param.lua");

    // Click on the parameter declaration `x` at col 11
    let h = document_highlight(&doc, pos(0, 11)).expect("Some");
    let t = triples(&h);
    assert_eq!(
        t,
        vec![
...
fn document_highlight_for_numeric_loop_var() {
    ...
    let (doc, _uri, _agg) = setup_single_file(src, "forn.lua");

    let h = document_highlight(&doc, pos(0, 4)).expect("Some");
    let t = triples(&h);
    assert_eq!(
        t,
        vec![
...
fn document_highlight_for_generic_loop_vars() {
    ...
    let (doc, _uri, _agg) = setup_single_file(src, "forg.lua");
...
fn document_highlight_function_declaration_name() {
    ...
    let (doc, _uri, _agg) = setup_single_file(src, "fndecl.lua");

    let h = document_highlight(&doc, pos(0, 15)).expect("Some");
    let t = triples(&h);
    assert_eq!(
        t,
        vec![
...
fn document_highlight_shadowing_respects_scope() {
    let src = "local x = 1\ndo\n  local x = 2\n  print(x)\nend\nprint(x)\n";
    let (doc, _uri, _agg) = setup_single_file(src, "shadow.lua");
    ...
    // inner usage only.
    let h = document_highlight(&doc, pos(2, 8)).expect("Some");
    let t = triples(&h);
    assert_eq!(
        t,
        vec![
...
fn document_highlight_global_variable() {
    ...
    let src = "G = 1\nG = G + 1\nprint(G)\n";
    let (doc, _uri, _agg) = setup_single_file(src, "global.lua");

    let h = document_highlight(&doc, pos(0, 0)).expect("Some");
    let t = triples(&h);
    assert_eq!(
        t,
        vec![
...
fn document_highlight_local_x_equals_x_rhs_not_new_local() {
    ...
    let src = "local x = 10\nlocal x = x + 1\nprint(x)\n";
    let (doc, _uri, _agg) = setup_single_file(src, "self_ref.lua");

    // Click on the NEW (inner) `x` at line 1 col 6
    let h = document_highlight(&doc, pos(1, 6)).expect("Some");
    let t = triples(&h);
    assert_eq!(
        t,
        vec![
...
fn document_highlight_indexed_assignment_base_is_read() {
    ...
    let src = "t = {}\nt.x = 1\nt[42] = 2\nprint(t)\n";
    let (doc, _uri, _agg) = setup_single_file(src, "indexed.lua");
...
fn document_highlight_no_match_on_nothing() {
    ...
    assert!(document_highlight(&doc, pos(0, 0)).is_none());
}
