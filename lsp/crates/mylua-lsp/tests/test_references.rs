...
fn references_local_rebind_does_not_claim_outer_rhs() {
    ...
    let inner_decl = result
        .iter()
        .find(|l| l.range.start.line == 2 && l.range.start.character == 8);
    assert!(
        inner_decl.is_some(),
        "should include inner decl itself: {:?}",
        result
    );

    let printed = result
        .iter()
        .find(|l| l.range.start.line == 3 && l.range.start.character == 8);
    assert!(
        printed.is_some(),
        "should include use inside the block: {:?}",
        result
    );

    let rhs_read = result
        .iter()
        .find(|l| l.range.start.line == 2 && l.range.start.character == 12);
    assert!(
        rhs_read.is_none(),
...
fn references_shadowed_outer_not_claimed_by_inner() {
    ...
    assert!(
        ...
        "outer use on line 1 should be present: {:?}",
        result,
    );
    assert!(
        result.iter().any(|l| l.range.start.line == 6),
        "outer use on line 6 (after do-block) should be present: {:?}",
        result,
    );
}
...
fn references_emmy_type_scans_annotations() {
    ...
    let defn_summary = summary_builder::build_file_analysis(
        &defn_uri,
        &defn_tree,
        defn_src.as_bytes(),
        &mylua_lsp::util::LineIndex::new(defn_src.as_bytes()),
    );
    let defn_doc = Document {
        lua_source: mylua_lsp::util::LuaSource::new(defn_src.to_string()),
        tree: Some(defn_tree),
        scope_tree: defn_summary.1,
...
Bar = {}"#;
    ...
    let user_result = summary_builder::build_file_analysis(
        &user_uri,
        &user_tree,
        user_src.as_bytes(),
        &mylua_lsp::util::LineIndex::new(user_src.as_bytes()),
    );
    let user_doc = Document {
        lua_source: mylua_lsp::util::LuaSource::new(user_src.to_string()),
        tree: Some(user_tree),
        scope_tree: user_result.1,
        last_diagnostic_signature: None,
    };
    ...
    let docs = HashMap::from([(defn_uri_id, defn_doc), (user_uri_id, user_doc)]);
...
print(myvar)"#;
    ...
    let with_decl = references::find_references(
        doc,
        intern_uri(&uri),
        pos(1, 6),
        true,
...
print(b.name)"#;
    ...
    let result = summary_builder::build_file_analysis(
        ...
        &tree,
        src.as_bytes(),
        &mylua_lsp::util::LineIndex::new(src.as_bytes()),
    );
    let doc = Document {
    ...
    assert!(
        name_refs.is_empty(),
        "hp references must not include 'name' accesses: {:?}",
        name_refs
    );
}
...
player.hp = 100"#;
    ...
    let (docs, agg, _parser) = setup_workspace(&[("player.lua", def_src), ("main.lua", use_src)]);
