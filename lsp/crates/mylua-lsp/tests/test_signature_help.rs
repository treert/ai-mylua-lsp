...
foo(1, "x")"#;
    ...
    let help = signature_help::signature_help(&doc, intern(&uri), pos(5, 7), &mut agg)
...
make(1, "x")"#;
    ...
    let doc = docs.get(&intern(&uri)).expect("main.lua document");

    let help = signature_help::signature_help(doc, intern(&uri), pos(1, 8), &mut agg)
...
foo(1, 2, 3)"#;
    ...
    let h = signature_help::signature_help(&doc, intern(&uri), pos(1, 4), &mut agg).unwrap();
    assert_eq!(h.active_parameter, Some(0));

    // After first comma
    let h = signature_help::signature_help(&doc, intern(&uri), pos(1, 7), &mut agg).unwrap();
    assert_eq!(h.active_parameter, Some(1));

    // After second comma
    let h = signature_help::signature_help(&doc, intern(&uri), pos(1, 10), &mut agg).unwrap();
...
foo({1, 2, 3}, 5)"#;
    ...
    let h = signature_help::signature_help(&doc, intern(&uri), pos(1, 12), &mut agg).unwrap();
    assert_eq!(
        h.active_parameter,
        Some(0),
...
work("hi")"#;
    ...
    let h = signature_help::signature_help(&doc, intern(&uri), pos(5, 6), &mut agg)
        .expect("signatureHelp should return Some");
...
foo{a=1, b=2, c=3}"#;
    ...
    let h = signature_help::signature_help(&doc, intern(&uri), pos(3, 15), &mut agg)
...
f:init()"#;
    ...
    let h = signature_help::signature_help(&doc, intern(&uri), pos(12, 7), &mut agg)
...
fn signature_help_returns_none_outside_call() {
    ...
    let h = signature_help::signature_help(&doc, intern(&uri), pos(0, 4), &mut agg);
...
"#,
    ...
    let doc = docs.get(&intern(&caller_uri)).expect("caller doc present");

    // Cursor inside `f:init(|)` on line 2, col 7.
    let h = mylua_lsp::signature_help::signature_help(doc, intern(&caller_uri), pos(2, 7), &mut agg)
        .expect("signatureHelp should resolve for f:init()");
    let labels: Vec<String> = h.signatures.iter().map(|s| s.label.clone()).collect();
...
handler:shout("hi")"#;
    ...
    let h = mylua_lsp::signature_help::signature_help(&doc, intern(&uri), pos(10, 14), &mut agg)
...
fn signature_help_local_anonymous_function_shows_ast_params() {
    ...
    let h = signature_help::signature_help(&doc, intern(&uri), pos(1, 2), &mut agg)
...
f(1, "x")"#;
    ...
    let h = signature_help::signature_help(&doc, intern(&uri), pos(4, 6), &mut agg)
...
f(1)"#;
    ...
    let h = signature_help::signature_help(&doc, intern(&uri), pos(3, 2), &mut agg)
...
o:move(10)"#;
    ...
    let h = signature_help::signature_help(&doc, intern(&uri), pos(8, 7), &mut agg);
    if let Some(h) = h {
