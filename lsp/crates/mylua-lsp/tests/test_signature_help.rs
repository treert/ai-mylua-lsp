...
foo(1, "x")"#;
    ...
    let help = signature_help::signature_help(&doc, intern_uri(&uri), pos(5, 7), &mut agg)
...
make(1, "x")"#;
    ...
    let help = signature_help::signature_help(doc, intern_uri(&uri), pos(1, 8), &mut agg)
...
foo(1, 2, 3)"#;
    ...
    let h = signature_help::signature_help(&doc, intern_uri(&uri), pos(1, 4), &mut agg).unwrap();
    assert_eq!(h.active_parameter, Some(0));

    // After first comma
    let h = signature_help::signature_help(&doc, intern_uri(&uri), pos(1, 7), &mut agg).unwrap();
    assert_eq!(h.active_parameter, Some(1));

    // After second comma
    let h = signature_help::signature_help(&doc, intern_uri(&uri), pos(1, 10), &mut agg).unwrap();
...
foo({1, 2, 3}, 5)"#;
    ...
    let h = signature_help::signature_help(&doc, intern_uri(&uri), pos(1, 12), &mut agg).unwrap();
    assert_eq!(
        h.active_parameter,
        Some(0),
...
work("hi")"#;
    ...
    let h = signature_help::signature_help(&doc, intern_uri(&uri), pos(5, 6), &mut agg)
        .expect("signatureHelp should return Some");
...
foo{a=1, b=2, c=3}"#;
    ...
    let h = signature_help::signature_help(&doc, intern_uri(&uri), pos(3, 15), &mut agg)
...
f:init()"#;
    ...
    let h = signature_help::signature_help(&doc, intern_uri(&uri), pos(12, 7), &mut agg)
...
fn signature_help_returns_none_outside_call() {
    ...
    let h = signature_help::signature_help(&doc, intern_uri(&uri), pos(0, 4), &mut agg);
...
fn signature_help_local_anonymous_function_shows_ast_params() {
    ...
    let h = signature_help::signature_help(&doc, intern_uri(&uri), pos(1, 2), &mut agg)
...
f(1, "x")"#;
    ...
    let h = signature_help::signature_help(&doc, intern_uri(&uri), pos(4, 6), &mut agg)
...
f(1)"#;
    ...
    let h = signature_help::signature_help(&doc, intern_uri(&uri), pos(3, 2), &mut agg)
