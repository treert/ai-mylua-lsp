...
fn inlay_hints_disabled_by_default() {
    ...
    let hints = inlay_hint::inlay_hints(&doc, intern(uri.clone()), full_range(), &mut agg, &cfg(false, true, true));
...
fn inlay_hints_parameter_names_at_call_site() {
    let src = "local function foo(a, b) end\nfoo(1, 2)\n";
    let (doc, uri, mut agg) = setup_single_file(src, "a.lua");
    let hints = inlay_hint::inlay_hints(&doc, intern(uri.clone()), full_range(), &mut agg, &cfg(true, true, false));
...
fn inlay_hints_variable_type_for_primitive() {
    let src = "local n = 42\n";
    let (doc, uri, mut agg) = setup_single_file(src, "a.lua");
    let hints = inlay_hint::inlay_hints(&doc, intern(uri.clone()), full_range(), &mut agg, &cfg(true, false, true));
    let labels: Vec<String> = hints.iter().map(label).collect();
    assert!(
...
fn inlay_hints_respect_range_filter() {
    ...
    let hints = inlay_hint::inlay_hints(&doc, intern(uri.clone()), range, &mut agg, &cfg(true, true, false));
