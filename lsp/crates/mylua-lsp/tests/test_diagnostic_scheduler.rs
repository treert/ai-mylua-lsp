...
use mylua_lsp::uri_id::{UriId, UriInterner};
...
fn id(interner: &UriInterner, s: &str) -> UriId {
    interner.intern(uri(s))
}
...
fn seed_bulk_hot_cold_split_preserves_priority_order() {
    ...
    s.seed_bulk(vec![id(&interner, "open1"), id(&interner, "open2")], Priority::Hot);
    s.seed_bulk(
        vec![id(&interner, "closed1"), id(&interner, "closed2"), id(&interner, "closed3")],
        Priority::Cold,
    );

    assert_eq!(s.pop(), Some(id(&interner, "open1")));
    assert_eq!(s.pop(), Some(id(&interner, "open2")));
    assert_eq!(s.pop(), Some(id(&interner, "closed1")));
    assert_eq!(s.pop(), Some(id(&interner, "closed2")));
    assert_eq!(s.pop(), Some(id(&interner, "closed3")));
...
fn seed_bulk_only_open_simulates_openonly_scope() {
    ...
    let s = DiagnosticScheduler::new();
    let interner = UriInterner::new();
    s.seed_bulk(vec![id(&interner, "open1")], Priority::Hot);
...
async fn schedule_then_upgrade_from_cold_via_open_flow() {
    ...
    let interner = UriInterner::new();
    s.seed_bulk(
        vec![id(&interner, "a"), id(&interner, "b"), id(&interner, "c"), id(&interner, "d"), id(&interner, "e")],
        Priority::Cold,
    );

    s.schedule(id(&interner, "c"), Priority::Hot);

    tokio::time::sleep(Duration::from_millis(400)).await;

    assert_eq!(s.pop(), Some(id(&interner, "c"))); // Hot 优先
    assert_eq!(s.pop(), Some(id(&interner, "a")));
    assert_eq!(s.pop(), Some(id(&interner, "b")));
    // uri "c" 的 cold tombstone 被跳过
    assert_eq!(s.pop(), Some(id(&interner, "d")));
    assert_eq!(s.pop(), Some(id(&interner, "e")));
    assert_eq!(s.pop(), None);
}
...
async fn rapid_schedule_collapses_to_single_push() {
    ...
    for _ in 0..10 {
        s.schedule(id(&interner, "a"), Priority::Hot);
...
fn invalidate_enables_re_enqueue_same_uri() {
    ...
    let s = DiagnosticScheduler::new();
    let interner = UriInterner::new();
    s.seed_bulk(vec![id(&interner, "a")], Priority::Hot);
    s.pop(); // 取出，enqueued 已清

    s.invalidate(&id(&interner, "a"));
    s.seed_bulk(vec![id(&interner, "a")], Priority::Cold);
    assert_eq!(s.pop(), Some(id(&interner, "a")));
}
