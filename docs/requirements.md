...
以下均须在 **全工作区** 语义下可用（在可配置的包含/排除与 `require` 解析规则之内），而非仅当前打开文件或依赖闭包：

- **转到定义**（含跨 `require` / 模块边界）。
- **查找所有引用**（`textDocument/references`）。
- **工作区符号**（`workspace/symbol`，全库搜索符号）。

另需支持基于 EmmyLua 的 **类型侧导航**（如 `@class` / `@alias` 名）作为与跳转一致体验的一部分。

- **模块解析**：须有可配置的 **`require` 解析策略**（`package.path` 模拟、项目根、路径别名等）；**无配置时**给出合理默认，**有配置时**明确覆盖关系。
...
- 变更 **架构、数据流、索引策略、文法边界或对外配置项** 时，同步更新 [`architecture.md`](architecture.md) 与本文。
