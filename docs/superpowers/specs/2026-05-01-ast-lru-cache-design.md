...
pub struct DocumentStore {
    documents: HashMap<UriId, Document>,
    /// LRU 访问顺序跟踪，只含非 pinned 且 has_ast 的 URI。
    /// front = 最久未使用，back = 最近使用。
    lru_tracker: VecDeque<UriId>,
...
- `HashMap<UriId, Document>` 签名基本不动，消费端改动极小
...
`lru_tracker` 是 `VecDeque<UriId>`，**只跟踪非 pinned 且 AST 在缓存中的文件**。pinned 文件不进 tracker。
...
insert / get_ast 后：
  如果 capacity > 0 且 lru_tracker.len() > capacity：
    while lru_tracker.len() > capacity:
      uriid = lru_tracker.pop_front()
      documents[uriid].tree = None
      documents[uriid].scope_tree = None
```
...
| 方法 | 用途 |
|------|------|
| `get(&self, uriid) → Option<&Document>` | 拿 Document（lua_source 始终有，tree 可能 None） |
| `get_with_ast(&mut self, uriid) → Option<&Document>` | 保证 tree/scope_tree 可用，miss 时自动重建 |
| `parse_temp(&mut self, uriid) → Option<Tree>` | 只做 tree-sitter parse，**不**写回 Document、**不**进 tracker。给 references/rename 全量扫描用 |
| `insert(uriid, lua_source, tree, scope_tree, parse_ms)` | 冷启动/编辑时写入 |
| `remove(uriid)` | 文件删除 |
| `set_open(uriid, bool)` | did_open / did_close 切换 |
...
```rust
// 之前
let doc = docs.get(uriid)?;
doc.tree.root_node()  // 直接用

// 之后 —— 大多数 handler
let doc = store.get_with_ast(uriid)?;
doc.tree.as_ref()?.root_node()  // 多一层 Option

// references 全量扫描
for (uriid, doc) in store.iter() {
    if !doc.lua_source.text().contains(name) { continue; }
    let tree = store.parse_temp(uriid)?;  // 临时 parse
    // ...
}
```
...
`docs.get(uriid)` → `store.get_with_ast(uriid)`，cache miss 自动重建。
...
for (uriid, doc) in store.iter() {
    ...
    let tree = store.parse_temp(uriid)?;
    // ... AST 扫描 ...
}
```
...
```
store.insert(uriid, lua_source, tree, scope_tree, elapsed_ms);
// insert 内部：
//   elapsed_ms > threshold → doc.is_slow = true
//   非 slow → lru_tracker.push_back(uriid)
```
...
```
// did_open
store.set_open(uriid, true);   // doc.is_open = true, 从 tracker 移除

// did_close
store.set_open(uriid, false);  // doc.is_open = false
                              // 非 slow → tracker.push_back(uriid)
```

### 8.5 锁粒度

`Arc<Mutex<HashMap<UriId, Document>>>` → `Arc<Mutex<DocumentStore>>`，粒度不变。

## 9. 已知局限

- **全量扫描与 LRU 的矛盾**：references 触发时需要对大量文件做 `parse_temp`，削弱了 LRU 的内存收益。
    - summary 记录引用反向索引，references 可缩小扫描范围，LRU 收益才能完全发挥。
- **parse_temp 代价**：tree-sitter parse 很快（大多数文件 < 1ms），但 2 万文件全量 parse_temp 仍需数秒。文本预筛可将实际 parse 数量降低一个数量级。
- **VecDeque 更新 O(n)**：`get_with_ast` cache hit 时需要在 VecDeque 中找到 URIID 并移到尾部，O(n)。对于 capacity=200 的规模，n 很小，不构成瓶颈。若后续需要优化可换 `IndexMap` 或侵入式链表。
