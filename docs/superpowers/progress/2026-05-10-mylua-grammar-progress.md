...
**日期**: 2026-05-10  
**状态**: P0/P1/P2 完成；P3 safe access/call 已收尾（含 standalone safe call statement）；P3 named/spread args 第一阶段已收尾；top_keyword corpus/default 差异已收尾；下一步处理 safe access 链式增强、keyword-as-name 完整范围或 `$function`
...
- 新增 `grammar/test/corpus/mylua.txt`
  - 当前覆盖并通过：`continue`、`array_constructor`、`t[] = value`、safe access/call、named/spread call arguments
- 新增 `lsp/crates/mylua-lsp/tests/test_mylua_parse.rs`
  - 已启用：低风险语法、`continue.mylua`、`array.mylua`、safe access/call、named/spread inline smoke test
...
cargo test parse_mylua_named_and_spread_args_syntax --test test_mylua_parse -- --nocapture
cargo test argument_count_skips_spread_argument_expansion --test test_diagnostics -- --nocapture
cargo test argument_type_uses_named_argument_value --test test_diagnostics -- --nocapture
cargo test
```
...
- `npx tree-sitter test`: 52/52 passed
- `cargo test parse_mylua_named_and_spread_args_syntax --test test_mylua_parse -- --nocapture`: passed
- `cargo test argument_count_skips_spread_argument_expansion --test test_diagnostics -- --nocapture`: passed
- `cargo test argument_type_uses_named_argument_value --test test_diagnostics -- --nocapture`: passed
- `cargo test`: passed
...
```text
 ...
 M lsp/crates/mylua-lsp/src/diagnostics/call_args.rs
 M lsp/crates/mylua-lsp/src/inlay_hint.rs
 M lsp/crates/mylua-lsp/src/util.rs
 M lsp/crates/mylua-lsp/tests/test_diagnostics.rs
 M lsp/crates/mylua-lsp/tests/test_inlay_hint.rs
...
1. safe access/call 可选增强
   - 可选：再支持链式 `obj?.field?.nested`
2. keyword-as-name 完整范围
   - 当前仅为 `continue` label/goto 做了最小适配
3. `$function`
4. `$string` scanner mode
   - 完成后可启用 `parse_mylua_named_args_fixture`（该 fixture 当前仍包含 `$"..."`）
5. named/spread 后续语义增强
