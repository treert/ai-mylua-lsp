...
- `lsp/crates/mylua-lsp/src/completion.rs`
  ...
  - 新增调用参数位置 named argument name completion：`foo(co|)` 在 `.mylua` 中基于已解析函数签名补 `count=`，并跳过 method call 隐式 `self` 与已使用的 named 参数；`.lua` 文件不启用该 MyLua 专属补全
- `lsp/crates/mylua-lsp/tests/test_completion.rs`
  ...
  - 新增 `complete_named_argument_names_from_call_signature`
  - 新增 `complete_named_argument_names_only_for_mylua_files`
...
- 提交上一项：`738a13c fix: suppress completion in dollar string text`。
- 本次继续 P6 completion / named args 细分场景：新增调用参数位置 named argument name completion 回归测试，先 RED（`foo(c|)` 中缺少来自签名参数 `count` 的候选），再复用 signature help 的 call signature resolution 返回 `count=` 插入文本；当前第一阶段仅在参数名前缀位置触发，已避免在已有 `name=value` 值区域、spread 参数和 `$string` 普通内容区触发。
- 根据补充限制补齐 `.lua` 负向回归：`complete_named_argument_names_only_for_mylua_files` 先 RED（`.lua` 中错误出现 `count=`），再增加 URI 后缀门禁，named argument name completion 仅在 `.mylua` 文件启用。
...
cargo test -p mylua-lsp --test test_completion complete_named_argument_names_from_call_signature -- --nocapture
cargo test -p mylua-lsp --test test_completion complete_named_argument_names_only_for_mylua_files -- --nocapture
...
- 提交前复验：`cargo test -p mylua-lsp --test test_completion` passed（13 passed），随后提交 `738a13c`。
- 新增 RED 验证：`complete_named_argument_names_from_call_signature` 初次失败，确认 `foo(c|)` 中缺少 `count` named argument 候选；修复后单测 passed。
- 新增 RED 验证：`complete_named_argument_names_only_for_mylua_files` 初次失败，确认 `.lua` 中错误出现 `count=` MyLua named argument 候选；加 `.mylua` URI 门禁后单测 passed。
- `rustfmt` 仅作用于本次 2 个 Rust 修改文件，passed。
- `cargo test -p mylua-lsp --test test_completion`: passed（15 passed）。
...
1. P6 LSP 基础能力稳定
   ...
   - completion 已补普通 `$string` 内容/`$$` 转义区不误触发通用 locals/globals/keywords 的回归；调用参数位置 named argument name completion 已补第一条回归且仅限 `.mylua`；后续可评估 `$name` / `${expr}` 内部补全策略、method call named 参数与已使用参数过滤的更多边界
