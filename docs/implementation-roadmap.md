...
- **语义诊断** 分级与后台调度固化。
- **索引生命周期**：`Initializing`/`Ready` 状态机 + `window/workDoneProgress` 进度通知 + `rayon` 并行冷启动 + 300ms 去抖诊断调度 + FNV 磁盘缓存（`DocumentSummary` 序列化）。
...
| **LSP 运行时** | **Rust** | 与 Tree-sitter C FFI 天然契合、单二进制分发、`spawn` 集成友好；**全工作区索引** 场景下内存与并发可控性好。 |
| **Extension** | **TypeScript** | VS Code 官方路径；thin client；通过子进程启动 LSP 二进制。 |
