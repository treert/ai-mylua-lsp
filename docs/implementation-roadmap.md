...
| **LSP 运行时** | **Rust 或 Go（优先 Rust）** | 与 `tree-sitter` 生态、FFI、单二进制分发友好；**全工作区索引** CPU 与内存更可控。 |
| **Extension** | **TypeScript** | VS Code 官方路径；thin；通过子进程启动 LSP 二进制。 |
| **若坚持用 TypeScript 写 LSP** | 须单独论证 | Tree-sitter `node-addon` 或子进程隔离、以及 **5 万文件索引** 的内存画像；不推荐作为默认「最佳方案」除非团队约束极强。 |

定稿后在本文 **删除「草案」措辞** 并记录 **具体语言与理由**。
