...
| 维度 | 现状 | 参考位置 |
|------|------|----------|
| **并行冷启动** | 5 阶段流水线：(1) scan 发现文件 + 建 module_entries → (1.5) 立即填充 module_index（`document_link` 可用）→ (2) `rayon` 全量并行 parse + build_summary → (3) 原子一次性 `build_initial` 构建全局索引 → (4) Ready。Phase 2 不持任何锁，Phase 1.5 短暂持 index 锁写入 module_index，Phase 3 单次临界区持 `open_uris → documents → index` 完成全量 merge（O(总贡献数)，20k 文件 ~几十 ms）。消除了旧 batch 模式下文件间 require 引用的时序依赖 bug。模块解析采用固定规则（全路径小写 + `.` 分隔 + `init.lua` 特殊处理 + last-segment 索引 + 最长后缀匹配），O(1) 查找无 fallback。`mylua/indexStatus` 通知携带 `phase` 字段（scanning / module_map_ready / parsing / merging），VS Code 状态栏分阶段显示 | `lib.rs::run_workspace_scan`（由 `initialized` 后台 spawn），`aggregation.rs::build_initial` |
| **并发安全的 scan/did_open 交错** | Phase 3 merge 阶段按 canonical `open_uris → documents → index` **整段持锁**；命中 `open_uris` 的 URI 跳过（buffer 版本优先），其 summary 从 `idx.summaries` 收集后一并传入 `build_initial`。Phase 2（并行 parse）期间 `did_open` / `did_change` 正常执行 `upsert_summary`（增量路径），Phase 3 的 `build_initial` 会清空并重建所有 shard，确保最终状态一致 | `lib.rs::run_workspace_scan`（Phase 3 merge block） |
...
| batch 模式下文件间 require 引用存在时序依赖 bug（早期 batch 的文件无法 resolve 后续 batch 的模块） | 重构为 5 阶段流水线：Phase 1 scan → Phase 1.5 立即填充 module_index → Phase 2 全量并行 parse（不持锁）→ Phase 3 原子 `build_initial` 一次性构建全局索引 → Phase 4 Ready。`build_initial` 先插入所有 summaries 再构建 shard，`resolve_module_to_uri` 在完整的 `module_index` + `summaries` 上运行，消除时序依赖。同时 `IndexStatusParams` 新增 `phase` / `message` 字段，VS Code 状态栏分阶段显示（scanning → module_map_ready → parsing N/M → merging → ready） | `aggregation.rs::build_initial`、`lib.rs::run_workspace_scan`、`extension.ts::renderStatus` |
| `resolve_module_to_uri` O(N) fallback 导致冷启动 O(N²) | 重新设计模块解析：移除 `require.paths` pattern 配置，改用固定规则（全路径小写 + `.` 分隔 + 去 `.lua` + `init.lua` 特殊处理）；`require_map: HashMap<String, Uri>` 改为 `module_index: HashMap<last_segment, Vec<(module_name, Uri)>>`，last-segment O(1) 查找 + 最长后缀匹配；别名替换改为最长前缀匹配；消除 summaries.keys() 全量遍历 fallback。Phase 1.5 在 scan 完成后立即填充 module_index，`document_link` 和 require 补全无需等待 parse | `workspace_scanner.rs`、`aggregation.rs`、`lib.rs`、`config.rs` |
