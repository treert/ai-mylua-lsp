...
| 维度 | 现状 | 参考位置 |
|------|------|----------|
| **并行冷启动** | 4 阶段流水线：(1) scan 发现文件 + 建 require_map → (2) `rayon` 全量并行 parse + build_summary → (3) 原子一次性 `build_initial` 构建全局索引 → (4) Ready。Phase 2 不持任何锁，Phase 3 单次临界区持 `open_uris → documents → index` 完成全量 merge（O(总贡献数)，20k 文件 ~几十 ms）。消除了旧 batch 模式下文件间 require 引用的时序依赖 bug。`mylua/indexStatus` 通知携带 `phase` 字段（scanning / parsing / merging），VS Code 状态栏分阶段显示 | `lib.rs::run_workspace_scan`（由 `initialized` 后台 spawn），`aggregation.rs::build_initial` |
...
| batch 模式下文件间 require 引用存在时序依赖 bug（早期 batch 的文件无法 resolve 后续 batch 的模块） | 重构为 4 阶段流水线：Phase 2 全量并行 parse（不持锁），Phase 3 原子 `build_initial` 一次性构建全局索引。`build_initial` 先插入所有 summaries 再构建 shard，`resolve_module_to_uri` 在完整的 `require_map` + `summaries` 上运行，消除时序依赖。同时 `IndexStatusParams` 新增 `phase` / `message` 字段，VS Code 状态栏分阶段显示（scanning → parsing N/M → merging → ready） | `aggregation.rs::build_initial`、`lib.rs::run_workspace_scan`、`extension.ts::renderStatus` |
