...
`tests/` 目录下的 Lua 文件**仅供手工调试**，可随意增删改。Rust 集成测试不依赖此目录。
...
采用 **lib + bin 拆分**：集成测试直接调用 `lib.rs` 导出的核心函数，无需 LSP stdio 通信。测试工具模块 `test_helpers.rs` 提供 `parse_doc()`、`setup_single_file()`、`setup_workspace()` 等辅助函数。Lua 测试源码全部内联在 test case 内。
