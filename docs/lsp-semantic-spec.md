...
| 侧 | 约束 | 原因 |
|---|---|---|
| 读 | 必须位于 chunk 的**直线执行流**上 | 函数体内的读，其执行时机与定义位置无关 |
| 写 | **每一处**写都必须位于同一条直线执行流上 | 出现函数体内 / 条件分支内的写，即放弃位置判定，视该字段为已定义 |

**什么算「直线执行流」**（`ScopeTree::is_on_chunk_straight_line`）：chunk 顶层作用域，**以及**嵌在其中的纯 `do … end` 块。`do` 块只引入词法作用域、不引入控制流，其语句照样按源码顺序恰好执行一次，位置判定完全成立；而 `do local _ENV = … end` 正是限定沙箱作用域的惯用写法，把它排除等于整类写法都不受检查。其余作用域一律不算：函数体（调用时机未知、甚至可能不调用）、`if`/`elseif`/`else` 分支（是否执行未知）、`while`/`repeat`/`for` 的 body（执行次数未知，**下一轮迭代**可以观察到源码里更靠后的写入）。注意 `while … do … end` 形如 `do` 块但不是 `do` 块，判据取自 `ScopeKind` 而非表面语法。
