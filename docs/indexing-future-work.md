...
> **定位**：本文档收集 `WorkspaceAggregation`（聚合层）与相关索引数据结构 / 泛型支持的**已知坑点**和**优化方向**，作为 `docs/future-work.md` 的领域专题补充。
>
...
> - 通用待办模板：[`future-work.md`](future-work.md)
...
- 每项落地后，在本文档对应条目下补 "**已完成于 <commit-hash / 日期>**"，并把条目从 `docs/future-work.md` 的"待办清单"中移除（若有）。
