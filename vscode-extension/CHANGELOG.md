...
### Fixed
- 类型未知的局部变量不再被当作同名全局变量：`obj.field` 里的 `obj` 若是当前作用域可见的局部变量、形参或循环变量，即使推不出类型，也不会再跳转 / hover 到其它文件里同名的全局符号。此前这类名字会兜底成全局引用，于是无 `---@param` 标注的形参（`function f(cfg)` 中的 `cfg.field`）、`for _, item in …` 的循环变量、以及赋值推不出类型的前置声明，其字段访问都可能落到工作区里毫不相关的同名全局上。现在这些情况一律按「类型未知」处理（不跳转、不提示），与「类型已知但解析不出」的既有行为一致。
