...
| [`vscode-extension/src/extension.ts`](vscode-extension/src/extension.ts) | LSP 客户端：启动 `mylua-lsp` 二进制（开发时自动查找 `lsp/target/debug/`）；创建 `MyLua` StatusBarItem 并订阅自定义通知 `mylua/indexStatus` —— **索引进行中显示 `💛X/Y`**（`X` 已索引、`Y` 总文件数；`total=0` 时回退为 `💛mylua`），**索引完成显示 `💚mylua`**；激活时先以 `💛mylua` 展示再等首个通知到达 |

**自定义 LSP 通知**：
- `mylua/indexStatus`（server → client，单向）：payload `{ state: "indexing" \| "ready", indexed: number, total: number }`。由 `lib.rs::run_workspace_scan` 在扫描开始、每批（50 文件）结束、以及 `IndexState::Ready` 后发出；扩展侧驱动 StatusBarItem。Rust 定义：`lib.rs::IndexStatusNotification` / `IndexStatusParams`。
