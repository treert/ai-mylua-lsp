...
- **并发安全**：每个 URI 一把 `Arc<tokio::sync::Mutex<()>>`（`Backend::edit_locks`），`did_open` / `did_change` / `did_close` 在处理前 `.await.lock()`（`did_close` 在锁内做磁盘读，防御性关闭 TOCTOU），防止同一文件的两次编辑在 `remove → process → insert` 的两阶段之间交错；不同 URI 并行不受影响。`edit_locks` / `diag_gen` 仅在 `did_change_watched_files` 的 DELETED 事件里清理（`did_close` 不清，方便下次 `did_open` 直接复用），工作区规模内容量可控
...
- **`did_open` 快速路径**（对称于 `did_close`）：若 `documents[uri].text` 与客户端传入的 buffer text 字节相等，直接 return，不 reparse / 不重建 summary / 不 republish 诊断。**只比 text 不比 version**（LSP version 在 reopen 时会被 client 重置为 1 但内容没变；不同 version 也可能内容一致，byte-equality 才是唯一安全信号）。冷启动把全工作区索引进 `documents` → 用户**首次**打开一个未改动的文件也命中；tab 切换来回开关同一文件也命中。上一次 publish 的诊断仍挂在 client（内容一致诊断一致），不会丢诊断；debounce 中的 semantic 任务若已在途，其发布仍对应当前 text，语义正确（fast path 不 bump `diag_gen`）。详见 [`docs/performance-analysis.md`](docs/performance-analysis.md) T1-1
