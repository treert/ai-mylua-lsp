...
内嵌单元测试（`src/*.rs` 中的 `#[cfg(test)] mod tests`）：`util.rs` 覆盖 UTF-8 ↔ UTF-16 列转换、LSP Position 转字节偏移、`apply_text_edit` 单行/跨行编辑的 `InputEdit` 构造；`lib.rs` 覆盖 `percent_decode` 的 UTF-8 多字节解码（中文路径）；`rename.rs` 覆盖 Lua 标识符校验；`signature_help.rs` 覆盖 `count_top_level_commas` 的嵌套 `{}`、未终止 `--[[` 不误计 trailing `,`、正确闭合的块注释等场景；`folding_range.rs` 覆盖长括号块注释前缀识别；`lua_builtins.rs` 覆盖 5.1/5.2/5.3/5.4/luajit 版本差异 builtin 列表；`workspace_scanner.rs` 含 `resolve_library_roots` 的空值/missing 路径剔除、canonical 去重、相对路径锚定首个 workspace root、无 workspace root 时的 bare relative drop 等单元测试；`workspace_scanner.rs`、`emmy.rs` 含大量单元测试覆盖模块路径推导、注解解析等。合计 **423 条测试**（cargo test --tests）全绿。
