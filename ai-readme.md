...
- `summary.rs`：`DocumentSummary` 含 `module_return_type`；`TypeDefinition` 含 `parents`（继承链）、`alias_type`（别名目标）、`name_range`（`@class/@alias/@enum <Name>` 的标识符 byte range）；`TypeFieldDef` 含 `name_range`（`@field <name>` 的标识符 byte range）；`FunctionSummary` 含 `overloads`；签名指纹包含全局类型信息和 module return
