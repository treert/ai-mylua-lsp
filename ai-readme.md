...
| [`vscode-extension/src/extension.ts`](vscode-extension/src/extension.ts) | LSP 客户端：启动 `mylua-lsp` 二进制（开发时自动查找 `lsp/target/debug/`）；创建 `MyLua` StatusBarItem 并订阅自定义通知 `mylua/indexStatus` —— **索引进行中显示 `💛X/Y`**（`X` 已索引、`Y` 总文件数；`total=0` 时回退为 `💛mylua`），**索引完成显示 `💚mylua`**；激活时先以 `💛mylua` 展示再等首个通知到达；`state=ready` 且携带 `elapsedMs` 时弹一次性 information toast "MyLua 索引完成，耗时 X.X 秒（N 个文件）"（session 内幂等：`readyNotified` 模块变量防止重复弹出） |
